...
func (e *Engine) DeleteBucketRangePredicate(ctx context.Context, orgID, bucketID influxdb.ID, min, max int64, pred influxdb.Predicate, opt influxdb.DeletePrefixRangeOptions) error {
	span, ctx := tracing.StartSpanFromContext(ctx)
	defer span.Finish()
	...
	return e.deleteBucketRangeLocked(ctx, orgID, bucketID, min, max, pred, opt)
}

// deleteBucketRangeLocked does the work of deleting a bucket range and must be called under
// some sort of lock.
func (e *Engine) deleteBucketRangeLocked(ctx context.Context, orgID, bucketID influxdb.ID, min, max int64, pred tsm1.Predicate, opt influxdb.DeletePrefixRangeOptions) error {
	// TODO(edd): we need to clean up how we're encoding the prefix so that we
	// don't have to remember to get it right everywhere we need to touch TSM data.
	encoded := tsdb.EncodeName(orgID, bucketID)
	name := models.EscapeMeasurement(encoded[:])

	return e.engine.DeletePrefixRange(ctx, name, min, max, pred, opt)
}
