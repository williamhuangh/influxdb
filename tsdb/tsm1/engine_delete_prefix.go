...
func (e *Engine) DeletePrefixRange(rootCtx context.Context, name []byte, min, max int64, pred Predicate, opt influxdb.DeletePrefixRangeOptions) error {
	...
	if len(possiblyDead.keys) > 0 && !opt.KeepSeries {
		buf := make([]byte, 1024)
