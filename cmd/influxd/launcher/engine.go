...
func (t *TemporaryEngine) DeleteBucketRangePredicate(ctx context.Context, orgID, bucketID influxdb.ID, min, max int64, pred influxdb.Predicate, opt influxdb.DeletePrefixRangeOptions) error {
	return t.engine.DeleteBucketRangePredicate(ctx, orgID, bucketID, min, max, pred, opt)
